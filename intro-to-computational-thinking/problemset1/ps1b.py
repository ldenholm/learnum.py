...
def dp_make_weight(egg_weights, target_weight, memo = {}):
    ...
    How can we tackle this problem and model it mathematically? The question is asking us to find a solution which    optimizes for the least number of eggs required to meet the weight constraint. I am considering representing 
    the problem with a binary search tree but examples I have seen use a recursive function instead.

    The issue with using a binary tree for this problem is finding the right way to represent decisions. In the 0/    1 knapsack problem each decision to add or ignore an item was a boolean choice. 
    ...
    G = (V, E) has Vertices with information and Edges represent the associated cost (in weight) of following a       path. If the path costs more than the desired weight then that path is not chosen.
